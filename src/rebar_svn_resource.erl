...
download_(Dir, {svn, Url}, State) ->
    rebar_api:warn("WARNING: It is recommended to use {branch, Name}, {tag, Tag} or {rev, Rev}, otherwise updating the dep may not work as expected.", []),
    download_(Dir, {svn, Url, {rev, "HEAD"}}, State);
download_(Dir, {svn, Url, {branch, Branch}}, _State) ->
    ok = filelib:ensure_dir(Dir),
    maybe_warn_local_url(Url),
    BranchUrl = make_branch_url(Url, Branch),
    rebar_utils:sh(lists:flatten(
                     io_lib:format("svn co -q ~ts ~ts",
...
download_(Dir, {svn, Url, {rev, Rev}}, _State) ->
    ...
    maybe_warn_local_url(Url),
    rebar_utils:sh(lists:flatten(
                     io_lib:format("svn co -q -r ~ts ~ts ~ts",
...
make_branch_url(Url, Branch) ->
    ...
    case lists:reverse(Path) of
        [_Branch, "branches"| Tail] ->
            rebar_api:warn("Specified branch (~ts) as well as a branch based URL. Using specified branch",
                  [Branch]),
        ...
        ["branches"| Tail] ->
            ...
            make_svn_url(Scheme, Host, Path1);
        ["trunk"| Tail] ->
            rebar_api:warn("Specified branch (~ts) on top of trunk URL. Using specified branch", [Branch]),
            Path1 = lists:reverse([Branch,  "branches" | Tail]),
            make_svn_url(Scheme, Host, Path1);
        RevPath ->
...
make_tag_url(Url, Tag) ->
    ...
    case lists:reverse(Path) of
        [_Tag, "tags"| Tail] ->
            rebar_api:warn("Specified tag (~ts) as well as a tag based URL. Using specified tag", [Tag]),
        ...
        ["tags"| Tail] ->
            ...
            make_svn_url(Scheme, Host, Path1);
        ["trunk"| Tail] ->
            rebar_api:warn("Specified tag (~ts) on top of trunk URL. Using specified tag", [Tag]),
            Path1 = lists:reverse([Tag,  "tags" | Tail]),
            make_svn_url(Scheme, Host, Path1);
        RevPath ->
...
maybe_warn_local_url(Url) ->
    try
        ...
        _:_ ->
            rebar_api:warn("URL format (~ts) unsupported.", [])
    end.
    ...
    case get({is_supported, ?MODULE}) of
        ...
        _ ->
            case rebar_utils:sh("svn --version", [{return_on_error, true},
                                                  {use_stdout, false}]) of
                {error, _} ->
                    rebar_api:abort("svn not installed", []);
                _ ->
